...
namespace Gibraltar.Monitor
{
    ...
    public class LogService : ILogService
    {
        ...
        private SessionSummary m_SessionStartInfo;
        private Publisher m_Publisher;         //Our one and only publisher
        private RepositoryPublishEngine m_PublishEngine; //We have zero or one publish engine
        private Packager m_ActivePackager; //PROTECTED BY LOCK
        private LocalRepository m_Repository;

        private readonly MetricDefinitionCollection m_Metrics = new MetricDefinitionCollection();

        private readonly ThreadLocal<ThreadInfo> m_TCurrentThreadInfo = new ThreadLocal<ThreadInfo>(); // ThreadInfo for the current thread, for efficiency.

        private bool m_SendSessionsOnExit; // protected by SYNCOBJECT

        private AgentConfiguration m_RunningConfiguration;

        private readonly object m_SyncObject = new object(); //the general lock for the log object.
        private readonly object m_InitializingLock = new object();
        private readonly object m_ConsentLock = new object(); //used just for the Auto Send Consent to keep it from interfering with anything else.
        private readonly object m_NotifierLock = new object(); //used for initializing Notifier instances.

        private volatile bool m_Initialized; //protected by being volatile
        private volatile bool m_InitializationNeverAttempted = true; //protected by being volatile
        private volatile bool m_Initializing; //PROTECTED BY INITIALIZING and volatile
        private volatile bool m_ExplicitStartSessionCalled; // protected by being volatile

        private Notifier m_MessageAlertNotifier; // PROTECTED BY NOTIFIERLOCK (weak check outside lock allowed)
        private Notifier m_MessageNotifier; // PROTECTED BY NOTIFIERLOCK (weak check outside lock allowed)
        private UserResolutionNotifier m_UserResolutionNotifier;

        // A thread-specific static flag for each thread to identify if this thread is the current initialize
        private readonly ThreadLocal<bool> m_TThreadIsInitializer = new ThreadLocal<bool>(); // false by default for each thread
        ...
        public bool IsLoggingActive()
        {
            ...
            if (m_InitializationNeverAttempted)
            {
                Initialize(null);
            }
        ...
        public bool Initialize(AgentConfiguration configuration)
        {
            ...
            lock(m_InitializingLock) //still need this lock to be sure we're the one and only thread that attempts to do initialization.
            {
                ...
                System.Threading.Monitor.PulseAll(m_InitializingLock);
            }

            if (performInitialization)
            {
                m_TThreadIsInitializer.Value = true; // so if we wander around and re-enter initialize we won't block.

                //we have to be sure that any initialization failure won't damage our lock state which would deadlock logging
                try
                {
                    m_Initialized = OnInitialize(configuration);
                }
                finally
                {
                    m_TThreadIsInitializer.Value = false;
                    m_InitializationNeverAttempted = false; // we no longer want people to call into us. this should be set before we release the waiting threads waiting on Initializing.

                    //and we are no longer trying to initialize.
                    lock(m_InitializingLock)
                    {
            ...
            else
            {
                ...
                if (PublisherSettings.QueryThreadMustNotBlock() == false)
                {
                    //it's initializing, but not yet complete - we need to stall until it is.
                    lock (m_InitializingLock)
                    {
                        while (m_Initializing) //the status is still indeterminate.
                        {
                            System.Threading.Monitor.Wait(m_InitializingLock);
                        }
            ...
            return m_Initialized; // Initialization is now done, report completion.
        }
        ...
        /// </summary>
        public Notifier MessageAlertNotifier
        {
            get
            {
                if (m_MessageAlertNotifier == null)
                {
                    lock (m_NotifierLock) // Must get the lock to make sure only one thread can try this at a time.
                    {
                        if (m_MessageAlertNotifier == null) // Double-check that it's actually still null.
                            m_MessageAlertNotifier = new Notifier(LogMessageSeverity.Warning, "Message Alert");
                    }
                }

                return m_MessageAlertNotifier; // It's never altered again once created, so we can just read it now.
            }
        }
        ...
        /// </summary>
        public Notifier MessageNotifier
        {
            get
            {
                if (m_MessageNotifier == null)
                {
                    lock (m_NotifierLock) // Must get the lock to make sure only one thread can try this at a time.
                    {
                        if (m_MessageNotifier == null) // Double-check that it's actually still null.
                            m_MessageNotifier = new Notifier(LogMessageSeverity.Verbose, "Messages", false);
                    }
                }

                return m_MessageNotifier; // It's never altered again once created, so we can just read it now.
            }
        }
        ...
        /// </summary>
        public UserResolutionNotifier UserResolutionNotifier
        {
            get
            {
                if (m_UserResolutionNotifier == null)
                {
                    lock (m_NotifierLock) // Must get the lock to make sure only one thread can try this at a time.
                    {
                        if (m_UserResolutionNotifier == null) // Double-check that it's actually still null.
                            m_UserResolutionNotifier = new UserResolutionNotifier(m_RunningConfiguration.Publisher.EnableAnonymousMode);
                    }
                }

                return m_UserResolutionNotifier; // It's never altered again once created, so we can just read it now.
            }
        }
        ...
        /// <returns></returns>
        public bool CanSendSessions(ref string message)
        {
            ...
            PackagerConfiguration packager = m_RunningConfiguration.Packager;
        ...
        /// <returns></returns>
        public bool CanSendSessionsOnExit(ref string message)
        {
        ...
        public bool IsEmailSubmissionConfigured(ref string message)
        {
            if (m_Initialized == false)
            {
                message = "Gibraltar is not currently enabled";
        ...
        /// <returns></returns>
        public bool IsHubSubmissionConfigured(ref string message)
        {
            if (m_Initialized == false)
            {
                message = "Gibraltar is not currently enabled";
            ...
            // Do we appear to have sufficient configuration?
            ServerConfiguration server = m_RunningConfiguration.Server;
        ...
        /// </summary>
        public bool CanFindPackager(ref string message)
        {
            if (message == null)
                message = string.Empty;
        ...
        public bool SendSessionsOnExit
        {
            get
            {
                lock (m_SyncObject)
                {
                    System.Threading.Monitor.PulseAll(m_SyncObject);

                    return m_SendSessionsOnExit;
                }
            }
        ...
        public bool ExplicitStartSessionCalled
        {
            get { return m_ExplicitStartSessionCalled; }
        }
        ...
        /// </remarks>
        public MetricDefinitionCollection Metrics
        {
        ...
        /// </summary>
        public bool IsSessionEnding { get { return CommonCentralLogic.IsSessionEnding; } }

        /// <summary>
        /// Reports whether EndSession() has completed flushing the end-session command to the log.
        /// </summary>
        public bool IsSessionEnded { get { return CommonCentralLogic.IsSessionEnded; } }
        ...
        public void Write(List<MetricSample> samples)
        {
            if (m_Initialized == false)
                return;
            ...
            m_Publisher.Publish(packetArray, false);
        }
        ...
        public void Write(MetricSample sample)
        {
            ...
            m_Publisher.Publish(new[] {sample.Packet}, false);
        }
        ...
        public Version AgentVersion
        {
            get
            {
                ...
                return m_SessionStartInfo.AgentVersion;
            }
        }
        ...
        public void Write(LogMessageSeverity severity, LogWriteMode writeMode, Exception exception, string category, string caption,
        ...
        /// <param name="args">A variable number of arguments referenced by the formatted description string (or no arguments to skip formatting).</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void Write(LogMessageSeverity severity, LogWriteMode writeMode, Exception exception, int skipFrames, string category, string caption,
            string description, params object[] args)
        {
            if (m_Initialized == false)
                return;
        ...
        /// <param name="args">A variable number of arguments referenced by the formatted description string (or no arguments to skip formatting).</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void Write(LogMessageSeverity severity, LogWriteMode writeMode, Exception exception, bool attributeToException, string category, string caption,
            string description, params object[] args)
        {
            if (m_Initialized == false)
                return;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void WriteMessage(LogMessageSeverity severity, LogWriteMode writeMode, int skipFrames, Exception exception,
        ...
        /// <param name="args">Optional.  A variable number of arguments to insert into the formatted description string.</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void WriteMessage(LogMessageSeverity severity, LogWriteMode writeMode, int skipFrames, Exception exception, bool attributeToException,
            string detailsXml, string caption, string description, params object[] args)
        {
            if (m_Initialized == false)
                return;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void RecordException(int skipFrames, Exception exception, string detailsXml, string category,
            bool canContinue)
        {
            if (m_Initialized == false)
                return;
        ...
        /// <param name="canContinue">True if the application can continue after this call, false if this is a fatal error
        /// and the application can not continue after this call.</param>
        public void RecordException(IMessageSourceProvider sourceProvider, Exception exception, string detailsXml,
        ...
        private void RecordException(IMessageSourceProvider sourceProvider, Exception exception, string detailsXml,
        ...
        /// <param name="description">Optional.  A multi-line description to use which can be a format string for the arguments.  Can be null.</param>
        /// <param name="args">Optional.  A variable number of arguments to insert into the formatted description string.</param>
        public void WriteMessage(LogMessageSeverity severity, LogWriteMode writeMode, string logSystem,
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void EndFile(string reason)
        {
            if (m_Initialized == false)
                return;
        ...
        /// <param name="reason">An optionally-declared reason for invoking this operation (may be null or empty).</param>
        public void EndFile(int skipFrames, string reason)
        {
            if (m_Initialized == false)
                return;
        ...
        public void EndSession(SessionStatus endingStatus, IMessageSourceProvider sourceProvider, string reason)
        {
            if (m_Initialized == false)
                return;

            if (endingStatus < SessionStatus.Normal || m_Publisher == null)
                return;
...
#pragma warning restore 4014
        ...
        /// <param name="reason">A simple reason to declare why the application is ending as Normal or as Crashed, or may be null.</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void EndSession(SessionStatus endingStatus, int skipFrames, string reason)
        {
            if (m_Initialized == false)
                return;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void StartSession(AgentConfiguration configuration, int skipFrames, string reason)
        {
            m_ExplicitStartSessionCalled = true;
        ...
        /// <param name="reason"></param>
        public void StartSession(AgentConfiguration configuration, IMessageSourceProvider sourceProvider, string reason)
        {
            m_ExplicitStartSessionCalled = true;
        ...
        public async Task<bool> SendSessions(SessionCriteria? criteria, Predicate<ISessionSummary> sessionMatchPredicate, bool asyncSend)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (IsHubSubmissionConfigured(ref message) && (await Packager.CanSendToServer().ConfigureAwait(false)).IsValid)
                    {
                        ...
                        ServerConfiguration config = m_RunningConfiguration.Server;
        ...
        public void SetSendSessionsOnExit(bool value)
        {
...
#pragma warning restore 4014
        ...
        /// <param name="writeMode">Whether to queue-and-return or wait-for-commit.</param>
        public void Write(IMessengerPacket[] packetArray, LogWriteMode writeMode)
        {
            //we explicitly are not checking initialized because we are part of the initialize.
            if (m_Publisher == null)
                return;
            ...
            foreach (IMessengerPacket packet in packetArray)
            {
                ...
                if (logPacket != null)
                    m_SessionStartInfo.UpdateMessageStatistics(logPacket);
            }

            if (writeMode == LogWriteMode.WaitForCommit)
            {
                m_Publisher.Publish(packetArray, true);
            }
            else
            {
                m_Publisher.Publish(packetArray, false);
        ...
        public IMessengerPacket MakeLogPacket(LogMessageSeverity severity, string logSystem, string category,
            ...
            string detailsXml, string caption, string description, params object[] args)
        {
            ...
            if (m_RunningConfiguration.Publisher.EnableAnonymousMode)
            {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(userName))
                {
                    ...
                    userName = m_SessionStartInfo.FullyQualifiedUserName;
                }
        ...
        private ThreadInfo GetCurrentThreadInfo()
        {
            ...
            if (m_TCurrentThreadInfo.Value == null)
            {
                //we don't have it, go and create it.  We rely on still being on the
                //thread in question so we can get OUR thread information
                m_TCurrentThreadInfo.Value = new ThreadInfo();
            }

            return m_TCurrentThreadInfo.Value;
        }

        /// <summary>
        /// Indicates if the calling thread is part of the log initialization process
        /// </summary>
        public bool ThreadIsInitializer
        {
            get { return m_TThreadIsInitializer.Value; }
            set { m_TThreadIsInitializer.Value = value; }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private string GetPackagerFileNamePath()
        {
            //assume the packager assembly is in our directory, what path would that be?
            string proposedPath = typeof(LogService).GetTypeInfo().Assembly.CodeBase;
        ...
        /// </summary>
        private async Task SendSessionData()
        {
            try
            {
                string packagerFileNamePath = GetPackagerFileNamePath();
                ...
                if ((IsHubSubmissionConfigured(ref message)) && (await Packager.CanSendToServer().ConfigureAwait(false)).IsValid)
                {
                    argumentString.AppendFormat("/s /w \"{0}\" /m server /p \"{1}\" /a \"{2}\" ",
                        ourPid, m_SessionStartInfo.Product, m_SessionStartInfo.Application);
                ...
                else
                {
                    ...
                    argumentString.AppendFormat("/s /w \"{0}\" /m email /p \"{1}\" /a \"{2}\" /d \"{3}\" ",
                        ourPid, m_SessionStartInfo.Product, m_SessionStartInfo.Application, m_RunningConfiguration.Packager.DestinationEmailAddress);

                    if (!string.IsNullOrEmpty(m_RunningConfiguration.Packager.FromEmailAddress))
                    {
                        argumentString.AppendFormat(" /f \"{0}\" ", m_RunningConfiguration.Packager.FromEmailAddress);
                    }
                }

                //finally, did they override the file path we're supposed to use?
                if (string.IsNullOrEmpty(m_RunningConfiguration.SessionFile.Folder) == false)
                {
                    argumentString.AppendFormat("/folder \"{0}\" ", m_RunningConfiguration.SessionFile.Folder);
                }
        ...
        private bool OnInitialize(AgentConfiguration configuration)
        {
            ...
            Log.InitializingEventHandler tempEvent = Initializing;
            ...
            m_RunningConfiguration = initialConfiguration;

            //if we're in debug mode then force the central silent mode option.
            if (m_RunningConfiguration.Publisher.EnableDebugMode)
                SilentMode = false;

            m_SessionStartInfo = new SessionSummary(m_RunningConfiguration);

            if (m_RunningConfiguration.SessionFile.Enabled)
                m_Repository = new LocalRepository(m_SessionStartInfo.Product, m_RunningConfiguration.SessionFile.Folder);

            //initialize our publisher
            string sessionName = string.Format(CultureInfo.InvariantCulture, "{0} {1} {2} {3}", m_SessionStartInfo.Product,
                m_SessionStartInfo.Application,
                m_SessionStartInfo.ApplicationVersion,
                DateTimeOffset.UtcNow.ToString("yyyy-MM-dd HH-mm-ss", CultureInfo.InvariantCulture));

            m_Publisher = new Publisher(sessionName, m_RunningConfiguration, m_SessionStartInfo);

            //record our session start info right now so we're sure it's the first packet we have.
            Write(m_SessionStartInfo.Packet);
            
            //initialize the listener architecture.
            Listener.Initialize(m_RunningConfiguration, true);
        ...
        private void EnsureSummaryIsAvailable()
        {
            //make sure we aren't in a race condition starting to initialize...
            if ((m_Initialized == false) 
                && ((m_RunningConfiguration == null) || (m_SessionStartInfo == null)))
            {
                //it's initializing, but not yet complete - we need to stall until it is.
                lock (m_InitializingLock)
                {
                    ...
                    m_RunningConfiguration = new AgentConfiguration();
                    m_SessionStartInfo = new SessionSummary(m_RunningConfiguration);
        ...
        private void GetConsentScope(out string productName, out string applicationName)
        {
        ...
        private void StartPublishEngine()
        {
            try
            {
                ...
                lock (m_SyncObject)
                {
                    if ((m_RunningConfiguration.Server.Enabled) && (m_RunningConfiguration.Server.AutoSendSessions))
                    {
                        if (m_PublishEngine == null)
                        {
                            m_PublishEngine = new RepositoryPublishEngine(m_Publisher, m_RunningConfiguration);
                        }

                        publishEngine = m_PublishEngine;

                        if (m_RunningConfiguration.Server.AutoSendOnError)
                        {
        ...
        /// </summary>
        private void ShutdownPublishEngine()
        {
        ...
        private void Packager_EndSend(object sender, PackageSendEventArgs args)
        {
            lock(m_SyncObject)
            {
                ...
                if (ReferenceEquals(caller, m_ActivePackager))
                {
                    //outstanding! we can clear our pointer now.
                    m_ActivePackager.EndSend -= Packager_EndSend;
                    m_ActivePackager = null;
                }
    ...
    /// <summary>
    /// Provide the DebugBreak method on the ILogService class, because interfaces can't have Conditional methods.
    /// </summary>
    public static class LogServiceExtensions
    {
        ...
        /// </summary>
        /// <param name="logService">The <see cref="ILogService"/> instance.</param>
        [Conditional("DEBUG")]
        public static void DebugBreak(this ILogService logService)
        {
            if (logService.BreakPointEnable && Debugger.IsAttached)
            {
